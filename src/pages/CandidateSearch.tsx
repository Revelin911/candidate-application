...
// const [loading, setLoading] = useState<string | null>(null);
  ...
  const saveToPotential = () => { 
    ...
    const parsePotentialCandidate: Candidate[] = savedCandidates? JSON.parse(savedCandidates): [];
    console.log(parsePotentialCandidate)

    if (currentUser && !parsePotentialCandidate.some(candidate => candidate.login === currentUser.login)) {
      parsePotentialCandidate.push(currentUser);
    }
     localStorage.setItem('savedCandidates', JSON.stringify(parsePotentialCandidate));
...
<div>
  ...
  <div>
    ...
    <button onClick={() => setCurrentUserIndex(currentUserIndex - 1)} disabled={currentUserIndex === 0}> - </button>
    <button onClick={saveToPotential} disabled={currentUserIndex >= allUsersList.length + 1}> + </button>
  </div>
  ) : (
